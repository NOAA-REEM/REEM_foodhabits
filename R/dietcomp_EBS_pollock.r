...
ppfile       <- "C:/src/foodlab_internal_data/BS_raw.csv.gz"   # raw Pred/prey diet file (RACEBASE query result)
pplookfile   <- "lookups/Alaska_PreyLookup_MASTER.csv"  # lookup file for grouping prey
preylook_col <- "EBS_POLLOCK"                      # Column name in pplookfile to use to group prey
...
predators <- c("W.pollock") #,"P.halibut")
...
source("stationcounts.r") # returns hauls dataframe
...
write.csv(o_vals,"results/out_diets_EBSpollock.csv",row.names=F)
